...
1. 每单固定量的话，是指下单总量固定，输出买卖十个价格的比例 （和为1），比例乘以总量就是各个单的量吗。如果总量不固定的话，是指动作加一个维度表示总量吗。

4. D的输入是s, a 状态动作对还是 sasasasa...trajectory啊。我理解的是 D的输入是sasasasa，然后每输入一段trajectory返回给策略网络一个reward, 所以策略网络实际上是sasasa...reward
